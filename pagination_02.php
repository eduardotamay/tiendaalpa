...
class Paginar
    {
        ...
        public function getDatos($p,$clv_busqueda) {
        ...
        public function crearLinks( $enlaces ) {
            ...
            $html = '<li class="page-item'.$clase.'"><a class="page-link" aria-label="Previous" href="?producto='.$this->clave_busq.'&limit='.$this->limite.'&page='.($this->pagina-1).'"><span aria-hidden="true">&#9664;</span></a></li>';  
            if($comienzo > 1) {
                $html  .= '<li class="page-item"><a class="page-link" href="?producto='.$this->clave_busq.'&limit='.$this->limite.'&page=1">1</a></li>';
        
            }  
            for($i = $comienzo ; $i <= $fin; $i++) {
                $clase  = ($this->pagina == $i ) ? "active" : "";
                $claseTexto  = ($this->pagina == $i) ? "active bg-primary text-white" : "";
                $html  .= '<li class="page-item'.$clase.'"><a class="page-link '.$claseTexto.'" href="?producto='.$this->clave_busq.'&limit='.$this->limite.'&page='.$i.'">'.$i.'</a></li>';
            }
            

            if($fin < $ultimo) {
                $html  .= '<li class="page-item"><a class="page-link" href="?producto='.$this->clave_busq.'&limit='.$this->limite.'&page='.$ultimo.'">'.$ultimo.'</a></li>';
            }   
            $clase  = ( $this->pagina == $ultimo ) ? "disabled" : "enabled";
            $html  .= '<li class="page-item'.$clase.'"><a aria-label="Next" class="page-link"  href="?producto='.$this->clave_busq.'&limit='.$this->limite.'&page='.($this->pagina+1).'"><span aria-hidden="true">&#9654;</span></a></li>';
            return $html;
        }
