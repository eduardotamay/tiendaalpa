...
if(isset($_SESSION['nombre_cliente'])){
    ...
    if (!$order) {
        header( "refresh:2;url=pago.php"); 
    }
    // var_dump($order->id);
    // var_dump($order->charges[0]->payment_method->service_name);
    // var_dump($order->charges[0]->payment_method->reference);
    // var_dump($order->amount/100);
    // var_dump($order->line_items[0]->quantity.
    // "- ".$order->line_items[0]->name."- $".floatval($order->line_items[0]->unit_price/100));
?>
<!doctype html>
<html lang="es">
<head>
        ...
        <div class="row" style="margin-top:20px; margin-bottom:80px;color:#666;">
            ...
            <div class="col-md-4">
                <div class="caja-blanca sticky-top" style="top:100px;">
                    ...
                    <hr>
                    <div class="row" style="margin-bottom:20px;">
                        <div class="col-6" style="color:black;font-size:1rem;font-weight: bold;">Oxxo Pay</div>
                        <div class="col-6" style="text-align:right;"><img src="../img/oxxo.png" style="height:28px;"></div>
                    </div>
                    <h6 style="text-align:center;font-size: 0.9rem;">Referencia de pago</h6>
                    <div style="font-weight: bolder;background-color: #eee; border-radius: 5px;width:80%; padding:10px;font-size:1.2rem;color:black;text-align:center;margin:15px auto;"><?php echo $order->charges[0]->payment_method->reference; ?></div>
                    <h4>Expira el:</h4>
                    <p class="expira" style="text-align: end;">
                        <?php 
                            echo date('d-m-Y H:i A',$order->charges[0]->payment_method->expires_at);
                        ?>
                    </p>
                    <p style="font-size:0.6rem;text-align: center; color:#666;font-weight: bolder;">OXXO cobrará una comisión adicional al momento de realizar el pago</p>
                    <p style="font-size:0.9rem;color:black; font-weight: bold;">Instrucciones:</p>
                    <ol style="font-size:0.7rem;color:black; text-align: justify;line-height: 1.2rem;">
                    ...
                    <p style="font-size:0.7rem;color:black; text-align: center; font-weight: bold;">Al completar estos pasos te enviaremos un correo confirmando el pago.</p>
