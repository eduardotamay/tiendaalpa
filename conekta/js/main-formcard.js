...
$(function(){
    ...
    $(".number").keyup(function(event){
      ...
      if(parseInt(number.substring(0, 2)) > 50 && parseInt(number.substring(0, 2)) < 56){
        ...
        selected_card = 2;     
      }else if(parseInt(number.substring(0, 2)) == 36 || parseInt(number.substring(0, 2)) == 38){
        selected_card = 2; 
    ...
    $(".year").keypress(function(event){
      ...
      $(".date_value_year").html($(this).val());
      if(event.keyCode == 8 && $(".year").val().length == 4){
        $(this).val(month);
      }
      
      if($(this).val().length === 0){
        $(".date_value_year").text("YY");
      }
    }).keydown(function(){
      $(".date_value_year").html($(this).val());
    }).focus(function(){
      $(".date_value_year").css("color", "white");
    });
  });